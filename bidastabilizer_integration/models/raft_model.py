...
class RAFTModel(Configurable, torch.nn.Module):
    ...
    def __init__(self):
        ...
        raft = importlib.import_module(
            "bidastabilizer_integration.third_party.RAFT.core.raft"
        )
        self.raft_utils = importlib.import_module(
            "bidastabilizer_integration.third_party.RAFT.core.utils.utils"
        )

        # print(thirdparty_raft_path)
        self.model_weights: str = f"{thirdparty_raft_path}/models/raft-things.pth"
        ...
        self.model = raft.RAFT(model_args).to(device)
