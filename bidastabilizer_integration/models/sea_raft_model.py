...
class SEARAFTModel(Configurable, torch.nn.Module):
    ...
    def __init__(self):
        ...
        sea_raft = importlib.import_module(
            "bidastabilizer_integration.third_party.SEA-RAFT.core.raft"
        )
        self.raft_utils = importlib.import_module(
            "bidastabilizer_integration.third_party.SEA-RAFT.core.utils.utils"
        )
        super().__init__()
        self.model_weights: str = f"{thirdparty_searaft_path}/models/Tartan-C-T-TSKH-spring540x960-S.pth"
        ...
        self.model = sea_raft.RAFT(model_args).to(device)
