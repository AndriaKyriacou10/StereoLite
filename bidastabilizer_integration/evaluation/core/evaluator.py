...
from bidastabilizer_integration.evaluation.utils.eval_utils import eval_batch
# from bidavideo.evaluation.utils.eval_utils import depth2disparity_scale, eval_batch
from bidastabilizer_integration.evaluation.utils.utils import (
...
class Evaluator(Configurable):
    ...
    @torch.no_grad() # no gradient tracking -> only inference
    def evaluate_sequence(
        self,
        model,
        model_stabilizer,
        test_dataloader: torch.utils.data.DataLoader, # full video sequence per iteration
        ...
        model.eval()
        per_batch_eval_results = [] # populate with one-metric dictionary per sequence
        ...
        for batch_idx, sequence in enumerate(tqdm(test_dataloader)):
            ...
            batch_dict["stereo_video"] = sequence["img"] # left and right image stack
            if not is_real_data:
                batch_dict["disparity"] = sequence["disp"][:, 0].abs() # no real data -> GT is available 
                ...
                # resizes left/right video via bilinear interpolation before stacking back together.
