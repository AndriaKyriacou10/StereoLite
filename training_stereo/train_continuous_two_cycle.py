...
TWO-CYCLE VARIANT: unlike train_continuous.py, the LR schedule does NOT run as a single OneCycleLR
across the entire run. At the cutoff step, a fresh OneCycleLR is built for the no-CV phase, so it gets
its own full-strength warmup + decay over its own step budget instead of inheriting the decayed tail
of the CV-phase schedule. The optimizer instance (and its Adam moment buffers) is left untouched across
the swap -- only the LR schedule resets.
'''
...
def fetch_optimizer(model, args):
    """Builds the AdamW optimizer and the first-cycle (CV-phase) OneCycleLR scheduler."""
    optimizer = torch.optim.AdamW(params=model.parameters(), lr=args.lr, weight_decay=args.weight_decay)

    scheduler = torch.optim.lr_scheduler.OneCycleLR(
        optimizer=optimizer, max_lr=args.lr, total_steps=args.cutoff_step,
        pct_start=min(args.warmup_steps / args.cutoff_step, 0.5), cycle_momentum=False, anneal_strategy='linear'
        )

    return optimizer, scheduler


def fetch_no_cv_scheduler(optimizer, args, no_cv_total_steps):
    """Builds the second-cycle (no-CV-phase) OneCycleLR scheduler, replacing the CV-phase scheduler
    at the cutoff. The optimizer instance is left untouched -- only the LR schedule resets, so the
    no-CV phase gets its own full-strength LR budget instead of the decayed tail of the first cycle."""
    lr_no_cv = args.lr_no_cv if args.lr_no_cv is not None else args.lr
    return torch.optim.lr_scheduler.OneCycleLR(
        optimizer=optimizer, max_lr=lr_no_cv, total_steps=no_cv_total_steps,
        pct_start=min(args.warmup_steps / no_cv_total_steps, 0.5), cycle_momentum=False, anneal_strategy='linear'
        )
...
def train(args):
    ...
    model = StereoLite(scale_right=args.scale_right, layer2=args.layer2).to(device)
    ...
    no_cv_total_steps = args.total_steps - args.cutoff_step
    ...
    if args.resume_ckpt:
        ...
        if step >= args.cutoff_step:
            # Resuming into the no-CV cycle: rebuild the SECOND-cycle scheduler instead of the
            # first-cycle one that fetch_optimizer() built above, then load its saved state into it.
            scheduler = fetch_no_cv_scheduler(optimizer, args, no_cv_total_steps)
    ...
    # Tracks whether the CV -> no-CV scheduler swap has already happened, so that resuming mid- or
    # post-cutoff doesn't rebuild (and discard the freshly-loaded state of) the no-CV scheduler a
    # second time on the very next loop iteration.
    scheduler_switched = step >= cut_off_step
    ...
    while keep_training:
        ...
        for idx, data in enumerate(train_loader):
            ...
            if not scheduler_switched and step >= cut_off_step:
                # Cutoff reached mid-run: swap in a fresh scheduler for the no-CV cycle. The
                # optimizer object (and its Adam moment buffers) is left untouched -- only the LR
                # schedule resets, giving the no-CV phase its own full-strength budget.
                scheduler = fetch_no_cv_scheduler(optimizer, args, no_cv_total_steps)
                scheduler_switched = True
                lr_no_cv = args.lr_no_cv if args.lr_no_cv is not None else args.lr
                logging.info(f"Cutoff reached at step {step}. Switched to no-CV cycle scheduler "
                             f"(total_steps={no_cv_total_steps}, peak_lr={lr_no_cv}).")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lr', type=float, default=0.0002, help='Peak LR for the CV-phase (first) cycle')
    parser.add_argument('--lr_no_cv', type=float, default=None, help='Peak LR for the no-CV-phase (second) cycle. Defaults to --lr if not set')
    parser.add_argument('--warmup_steps', type=int, default=6000, help='Warmup iterations for each cycle (matches the ReCoVEr paper convention); pct_start is computed per-cycle as warmup_steps / cycle_total_steps')
