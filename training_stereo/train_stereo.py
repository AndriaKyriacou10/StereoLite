''' Train StereoLite Model with Two-Phase Training: 
    Phase 1 (Supervised against Ground Truth) 
...
def train_one_epoch_phase1(model:StereoLite, optimizer, scheduler, dataloader, device, scaler, logger=None):
    model.train()
...
def train_one_epoch_phase2(teacher_model:StereoLite, student_model:StereoLite, optimizer, scheduler, dataloader, device, scaler, logger=None):
    student_model.train()
...
def phase1_training(epochs, scale_right_weight):
    ...
    # Load Model
    model = StereoLite(scale_right=scale_right_weight).to(device)
...
def phase2_training(epochs, scale_right_weight, val_freq = 2500, teacher_ckpt='./checkpoints/phase1_best_model.pth'):
    ...
    teacher_model = StereoLite(scale_right=scale_right_weight).to(device)
    student_model = StereoLite(scale_right=scale_right_weight).to(device)
