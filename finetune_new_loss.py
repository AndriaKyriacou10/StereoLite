...
def smooth_l1_loss(disp_preds, disp_gt, valid, max_disp=192, keep_ratio=0.25):
    """
    Online Hard Example Mining: Focus the loss on the most difficult pixels in the crop.
    ...
    for i, disp_pr in enumerate(disp_preds):
        if disp_pr.dim() == 4:
            disp_pr = disp_pr.squeeze(1) # [B, H, W]
            
        # Calculate the raw element-wise error (NO reduction yet)
        diff = F.smooth_l1_loss(disp_pr[valid], disp_gt[valid], reduction='none')
        
        if diff.numel() > 0:
            # Determine how many pixels make up the top 'keep_ratio' (75%)
            k = int(diff.numel() * keep_ratio)
            
            if k > 0:
                # Sort and keep ONLY the worst errors
                diff_hard, _ = torch.topk(diff.view(-1), k)
                
                # Average the loss only over these difficult pixels
                loss += weights[i] * diff_hard.mean()
            
    return loss


def validate(model, val_loader, device, max_disp=192):
    """ Runs evaluation on the validation set without tracking gradients """
    model.eval()
    total_epe = 0
    total_bad2 = 0
    ...
    with torch.no_grad():
        for batch_idx, data in enumerate(tqdm(val_loader, desc="Validating")):
            ...
            # Forward pass in test mode (no intermediate predictions)
            disp_pred = model(img1, img2, max_disp=max_disp, test_mode=True)
            ...
            mask = (valid_mask >= 0.5) & (gt_disp > 0) & (gt_disp < max_disp)
            
            if mask.sum() > 0:
                abs_err = torch.abs(disp_pred[mask] - gt_disp[mask])
                
                total_epe += abs_err.sum().item()
                total_bad2 += (abs_err > 2.0).sum().item()
    ...
    avg_bad2 = (total_bad2 / total_valid_pixels) * 100.0
    
    return avg_epe, avg_bad2


def main():
    num_epochs = 250
    physical_batch_size = 4        
    effective_batch_size = 64      # Target batch size to simulate multi-GPU
    ...
    root_path = './data/datasets/Middlebury'
    ...
    # Split datasets into train and validation sets (90 - 10 split)
    ...
    logging.info(f"Data Split: {train_size} training pairs, {val_size} validation pairs.")
    
    train_loader = DataLoader(train_dataset, batch_size=physical_batch_size, shuffle=True, num_workers=8, drop_last=True, pin_memory=True)
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, data in enumerate(train_loader):
            ...
            # Compute scaled loss
            loss = smooth_l1_loss(disp_preds, gt_disp, valid_mask, keep_ratio = 0.5)
        ...
        #  VALIDATION
        val_epe, val_bad2 = validate(model, val_loader, device, max_disp)
        print(f"--- Validation Results | EPE: {val_epe:.4f} | Bad2: {val_bad2:.2f}% ---")

        if val_bad2 < best_bad2:
            best_bad2 = val_bad2
            save_path = f"checkpoints/liteanystereo_best_bad2_new.pth"
            torch.save(model.state_dict(), save_path)
            print(f"*** New Best Model Saved! (Bad2: {val_bad2:.2f}%) ***")
        else:
            patience_counter += 1
        ...
        if (epoch + 1) % 5 == 0 or (epoch + 1) == num_epochs:
            save_path = f"checkpoints/liteanystereo_finetune_ep{epoch+1}.pth"
